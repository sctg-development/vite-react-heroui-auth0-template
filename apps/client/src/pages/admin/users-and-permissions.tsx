...
export default function UsersAndPermissionsPage() {
            ...
            .then(async (resp) => {
                if ("access_token" in resp) {
                    ...
                    try {
                        ...
                        addToast({ title: t("error"), description: t("admin-users-toast-error-loading-users"), variant: "solid" });
                    }
                } else {
                    addToast({ title: t("error"), description: t("admin-users-toast-no-management-token"), variant: "solid" });
                }
            })
            .catch((err) => {
    ...
    const openUserEditing = async (userId: string) => {
        ...
        try {
            ...
            addToast({ title: t("error"), description: t("admin-users-toast-error-loading-perms"), variant: "solid" });
        } finally {
    ...
    const savePermissions = async (userId: string) => {
        ...
        try {
            ...
            addToast({ title: t("success"), description: t("admin-users-toast-success-update"), variant: "solid", timeout: 4000 });
    ...
    const deleteUser = async (userId: string) => {
        ...
        if (userId === currentUserId) {
            addToast({ title: t("error"), description: t("admin-users-toast-cannot-delete-self"), variant: "solid" });
            return;
        }
        if (!window.confirm(t("admin-users-confirm-delete-prefix", { userId }))) return;
        try {
            await deleteAuth0User(mgmtToken, userId);
            // Locally update the UI to remove the deleted user
            setUsers((prev) => prev.filter((u) => u.user_id !== userId));
            if (selectedUserId === userId) setSelectedUserId(null);
            addToast({ title: t("success"), description: t("admin-users-toast-success-delete"), variant: "solid" });
        } catch (err) {
            console.error("Error deleting user:", err);
            addToast({ title: t("error"), description: t("admin-users-toast-error-delete"), variant: "solid" });
        }
    };
            ...
            <section className="flex flex-col gap-6 py-8 md:py-10 px-4">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-default-500 text-sm mt-1">
                            ...
                            {tokenFromCache && (
                                <span className="ml-2 text-success-600 text-xs">{t("admin-users-cache-token-success")}</span>
                            )}
                        </p>
                    </div>
                    {isUpToDate === true ? (
                ...
                {selectedUserId && (
                    <div className="mt-6 p-6 border border-default-200 rounded-xl bg-default-50">
                        ...
                        {modalLoading ? (
                            <p className="text-default-500">{t("admin-users-modal-loading-perms")}</p>
                        ) : (
                            <>
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-6">
                                    {Permission.map((permValue) => (
                                        <Checkbox
                                            ...
                                            onValueChange={() => togglePermission(selectedUserId, permValue)}
                                            size="sm"
                                            isDisabled={
                                                // Prevent removing one's own auth0:admin:api permission
                                                // to avoid getting locked out of this page
                                                selectedUserId === currentUserId && permValue === "auth0:admin:api"
                                            }
                                        >
                                            <span className="text-xs">{t(`permission-${permValue.replace(/:/g, "-")}`, permValue)}</span>
                                        </Checkbox>
                                    ))}
                                </div>
